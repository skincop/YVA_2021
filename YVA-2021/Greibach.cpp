...
#pragma region GRB_rules	
	Greibach greibach(NS('S'), TS('$'),
		...
		Rule(NS('S'), GRB_ERROR_SERIES + 0, 3,						// Неверная структура программы	
			Rule::Chain(6, TS('t'), TS('f'), TS('i'), NS('A'), NS('C'), NS('S')),
			Rule::Chain(2, TS('m'), NS('C')),
			Rule::Chain(7, TS('d'),TS('t'), TS('f'), TS('i'), NS('A'), NS('C'), NS('S'))

		),

		Rule(NS('C'), GRB_ERROR_SERIES + 2, 2,					// Ошибка в теле функции
			Rule::Chain(5, TS('{'), TS('r'), NS('V'), TS(';'), TS('}')),
			Rule::Chain(6, TS('{'), NS('K'), TS('r'), NS('V'), TS(';'), TS('}'))
		),

		Rule(NS('A'), GRB_ERROR_SERIES + 1, 2,					// Не найден список параметров функции	
		...
		Rule(NS('P'), GRB_ERROR_SERIES + 6, 4,					// Ошибка в списке параметров функции		
			Rule::Chain(1, TS('i')),
			Rule::Chain(1, TS('l')),
			Rule::Chain(3, TS('i'), TS(','), NS('P')),
			Rule::Chain(3, TS('l'), TS(','), NS('P'))
		),
		...
		Rule(NS('U'), GRB_ERROR_SERIES + 9, 7,					// Неверный арифметический оператор
			Rule::Chain(1, TS('+')),
			Rule::Chain(1, TS('-')),
			Rule::Chain(1, TS('*')),
			Rule::Chain(1, TS('/')),
		...
		Rule(NS('W'), GRB_ERROR_SERIES + 11, 10,					// Ошибка в арифметичском выражении
			Rule::Chain(1, TS('i')),
			Rule::Chain(2, TS('i'),TS('B')),
			Rule::Chain(1, TS('l')),
			Rule::Chain(2, TS('l'), TS('B')),
			Rule::Chain(3, TS('('), NS('W'), TS(')')),
			Rule::Chain(5, TS('('), NS('W'), TS(')'), NS('U'), NS('W')),
			Rule::Chain(2, TS('i'), NS('F')),
			Rule::Chain(3, TS('i'), NS('U'), NS('W')),
			Rule::Chain(4, TS('i'), NS('F'), NS('U'), NS('W')),
			Rule::Chain(3, TS('l'), NS('U'), NS('W'))
		),
